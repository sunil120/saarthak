 ...
 */
function ya_unregister_default_widgets() {
	     unregister_widget('WC_Widget_Featured_Products');
		 unregister_widget('WC_Widget_Best_Sellers');
		 unregister_widget('WC_Widget_Top_Rated_Products');
}
 add_action('widgets_init', 'ya_unregister_default_widgets', 11);
 
function ya_widgets_init() {
	// Sidebars
	global $ya_widget_areas;
	$ya_widget_areas = ya_widget_setup_args();
	if ( count($ya_widget_areas) ){
		foreach( $ya_widget_areas as $sidebar ){
...
class YA_Social_Widget extends WP_Widget{
	...
	function form($instance){ //var_dump($instance);
		...
		?>
<p>
	<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title: ', 'yatheme'); ?>
...
<?php //var_dump($socials);
		...
		foreach ($socials as $j => $social){
			...
			?>
<p>
	<strong><?php echo __( 'Social', 'yatheme' ). ' ' .$j+1; ?>
	</strong><br>
	<?php if (key_exists('icon', $social)){?>
	<label for="<?php echo esc_attr( $id_j_icon ); ?>"><?php esc_html_e('fa-* | title', 'yatheme')?>
	...
	<?php }?>
	<label for="<?php echo esc_attr( $id_j_link ); ?>"><?php esc_html_e('Link ', 'yatheme')?> </label>
...
<?php
		...
		?>
<p>
	<strong><?php esc_html_e( 'Add a new social network', 'yatheme' ) ?></strong><br> <label
		for="<?php echo esc_attr( $id_i_icon ); ?>"><?php esc_html_e('Classname for Icon or Title', 'yatheme'); ?>
		...
		icon, please choose class name in Font Awesome. This is required.', 'yatheme' ) ?></span>
	<label for="<?php echo esc_attr( $id_i_link ); ?>"><?php esc_html_e('Link ', 'yatheme')?> </label>
	...
	public function in_widget_form($widget, $r = null, $instance = array() ){
		...
		<div class="advanced-opt <?php echo $adoptions_on_class; ?>">
			<div class="advanced-opt-controls" >
				<label class="button toggle" for="<?php echo esc_attr( $widget->get_field_id('adoptions') ); ?>"><?php esc_html_e( 'Advanced Options', 'yatheme' ) ?>
			...
			<div class="advanced-opt-pane">
				<div class="advanced-opt-pane-inner">
					<div class="pane-content">
						<div class="pane-left">
							<p>
								<label for="<?php echo esc_attr( $widget->get_field_id('widget_style') ); ?>"><?php esc_html_e( 'Widget Style', 'yatheme' ) ?>
...
//		return $this->menu_assignments[$hash];
	...
	private function render_wdisplayxx($widget_display){
		...
		<p>
			<label
				for="<?php echo esc_attr( $this->widget->get_field_id('display_select') ); ?>"><?php esc_html_e( 'Widget Display', 'yatheme' ) ?>
			</label>
		...
		<p><strong><?php esc_html_e( 'Users', 'yatheme' ) ?></strong><br/>
		<?php
		...
		<p><strong><?php esc_html_e( 'General', 'yatheme' ) ?></strong><br/>
		<?php
		foreach ($display_checkbox['general'] as $value => $name) { ?>
			...
			<input type="checkbox" id="<?php esc_attr( $this->widget->get_field_id($value) )?>"
				...
				/>
			<label for="<?php echo esc_attr( $this->widget->get_field_id($value) ); ?>"><?php esc_html_e( $name, 'yatheme' ) ?></label><br/>
		<?php } ?>
		</p>
		<p><strong><?php esc_html_e( 'Categories', 'yatheme' ) ?></strong><br/>
		<?php
		...
		<?php } ?>
				...
				<p><strong><?php esc_html_e( 'Pages', 'yatheme' ) ?></strong><br/>
		<?php
