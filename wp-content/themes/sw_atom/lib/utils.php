...
function ya_typography_webfonts(){
	if ( ya_options()->getCpanelValue('google_webfonts') ):
		...
		if ($f > 3){
			...
			if ( 'off' !== _x( 'on', 'Google font: on or off', 'yatheme' ) ) {
...
if ( ! function_exists( 'wp_site_icon' ) ) {
	function ya_add_favicon(){
		if ( ya_options()->getCpanelValue('favicon') ){
			echo '<link rel="shortcut icon" href="' . esc_url( ya_options()->getCpanelValue('favicon') ). '" />';
		}
	}
	add_action('wp_head', 'ya_add_favicon');
}
