...
class woocsv_import_product
{
    ...
    */
    ...
    public function __construct (Allaerd\Woocsv\LogToFile $logger)
    {
        $this->logger = $logger;
    }
    ...
    public function handlePrices ()
    {
        ...
        if ($woocsv_import->get_merge_products() == 1) {
            $regular_price = (in_array( 'regular_price',
                    $this->header ) && strlen( $this->meta[ '_regular_price' ] ) > 0) ? $this->meta[ '_regular_price' ] : $this->meta[ '_regular_price' ];
            $sale_price = (in_array( 'sale_price',
                    $this->header ) && strlen( $this->meta[ '_sale_price' ] ) > 0) ? $this->meta[ '_sale_price' ] : $this->meta[ '_sale_price' ];
            $price = (in_array( 'price',
                    $this->header ) && strlen( $this->meta[ '_price' ] ) > 0) ? $this->meta[ '_price' ] : $this->meta[ '_price' ];
        } else {
            $regular_price = (in_array( 'regular_price',
                    $this->header ) && strlen( $this->meta[ '_regular_price' ] ) > 0) ? $this->meta[ '_regular_price' ] : '';
            $sale_price = (in_array( 'sale_price',
                    $this->header ) && strlen( $this->meta[ '_sale_price' ] ) > 0) ? $this->meta[ '_sale_price' ] : '';
            $price = (in_array( 'price',
                    $this->header ) && strlen( $this->meta[ '_price' ] ) > 0) ? $this->meta[ '_price' ] : '';
        }
        ...
        foreach ($this->categories as $category) {
            ...
            foreach ($cats as $cat) {
                ...
                foreach ($cat_taxs as $cat_tax) {
                    ...
                    if (!is_array( $new_cat )) {
                        $new_cat = wp_insert_term( $cat_tax, 'product_cat',
                            array ( 'slug' => $cat_tax, 'parent' => $parent ) );
                    }
    ...
    public function fill_in_data ()
    {
        ...
        if (in_array( 'ID', $this->header )) {
            ...
            if ($tempID) {
                ...
                if ($test->post) {
                    $this->logger->log( sprintf( __( 'Product found (ID), ID is: %s', 'woocommerce-csvimport' ),
                        $tempID ) );
                    $this->new = FALSE;
        ...
        if (isset($this->body[ 'post_author' ]) && in_array( 'post_author', $this->header )) {
            ...
            $user = get_user_by( ($woocsv_import->get_match_author_by()) ? $woocsv_import->get_match_author_by() : 'login',
    ...
    {
        if (in_array( 'stock_status', $this->header ) && !in_array( $this->meta[ '_stock_status' ],
    ...
    {
        if (in_array( 'visibility', $this->header ) && !in_array( $this->meta[ '_visibility' ],
    ...
    {
        if (in_array( 'backorders', $this->header ) && !in_array( $this->meta[ '_backorders' ],
                array ( 'yes', 'no', 'notify' ) )
        ) {
    ...
    {
        if (in_array( 'manage_stock', $this->header ) && !in_array( $this->meta[ '_manage_stock' ],
                array ( 'yes', 'no' ) )
        ) {
