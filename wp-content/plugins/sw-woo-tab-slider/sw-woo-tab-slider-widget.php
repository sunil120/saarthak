<?php
/**
 * Plugin Name: SW Woo Tab Slider Widget
...
add_action( 'widgets_init', 'sw_woo_tab_slider' );

/**
 * Register our widget.
 * 'Slideshow_Widget' is the widget class used below.
 */
function sw_woo_tab_slider() {
...
class sw_woo_tab_slider_widget extends WP_Widget {
	 ...
	 * Widget setup.
	 */
	function sw_woo_tab_slider_widget() {
		/* Widget settings. */
	...
	**/
	function SC_integrateWithVC(){
		...
		$term = array( 'Alll Categories' => '' );
		foreach( $terms as $cat ){
			$term[$cat->name] = $cat -> term_id;
		}		
		vc_map( array(
			"name" => __( "Woocommerce Tab Slider", "smartaddons" ),
			"base" => "woo_tab_slider",
			"icon" => "icon-wpb-ytc",
			"class" => "",
			...
			"params" => array(
			 ...
			 array(
				...
				"holder" => "div",
				"class" => "",
				"heading" => __( "Show Readmore", "smartaddons" ),
				"param_name" => "show_more",
				"value" => array( 'Yes' => 'yes', 'No' => 'no' ),
				"description" => __( "Show readmore category", "smartaddons" )
			 ),
			array(
				"type" => "dropdown",
				"holder" => "div",
	...
	**/
	...
	public function form( $instance ) {
		...
		$select_order    	= isset( $instance['select_order'] )     ? strip_tags($instance['select_order']) : array();		
		...
		<p>
			<label for="<?php echo $this->get_field_id('show_more'); ?>"><?php _e("Show More Category", 'smartaddons')?></label>
			<br/>
